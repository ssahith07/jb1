...
//           <li className='text-white py-1'><Link to="/sign-up" >Signup</Link></li>
//         </ul>
//       </div>
//     </header>
//   )
...
const Navbar = () => {
  ...
  return (
    <header className="max-w-screen-2xl container mx-auto xl:px-24 px-4">
      <nav className="flex justify-between sticky items-center py-6">
        <a href="/" className="flex items-center gap-2 text-2xl text-black">
          <span>JobFusion</span>
        </a>
        {/* nav items for large display */}
        ...
        <div className="text-base text-primary font-medium space-x-5 hidden lg:block">
        {token ? (
            // If there's a token, display the logout button
            <Link to="/login" onClick={handleLogout}  className="py-2 px-5 border rounded hover:bg-red-500 hover:text-white">
              Logout
            </Link>
          ) : (
            // If there's no token, display the login and signup links
            <>
              <Link to="/login" className="py-2 px-5 border rounded hover:bg-gray-200">
                Login
              </Link>
        ...
        <ul>
          ...
          <li className="text-white py-1">
            {token ? (
              ...
              // If there's no token, display the login link
              <Link to="/login" onClick={handleMenuToggler}>
                Login
              </Link>
            )}
